...
### Fixed: header overflow, stray skill-hyphen artifact, and fabricated bullets (found via a real side-by-side diff)

Caught by comparing a real source resume against a fresh tailored output,
bullet-by-bullet — item (1) below was confirmed as reproducing in the
current renderer; item (3) (visual hierarchy) was checked the same way and
turned out to **already be correct** in the code (see note below); item (4)
is the safety-critical fix.

1. **Header contact line overflow, fixed.** `drawResumePdf()`'s contact line
   used to be a single centered `doc.text()` call with no width check, so a
   contact line with many fields (email + phone + location + linkedin +
   github + portfolio) ran off the page edge — reproduced directly (`umass.edu`
   truncated to `ass.edu`, portfolio URL cut off mid-string). Fixed by
   measuring with `doc.getTextWidth()` before drawing: if the full line fits
   in `PDF_CONTENT_WIDTH_PT`, draw it as one line same as before; if not,
   split into two centered lines (location on its own line under the name,
   email/phone/links below that) via a new `drawCenteredContactLine()`
   helper, which itself falls back to `splitTextToSize()` wrapping if even
   the second line is still too wide. Verified by rendering the actual
   reported contact string in isolation (Node + the bundled jsPDF, no
   browser needed) — confirmed both the old code overflows and the new code
   wraps cleanly to two lines with nothing cut off.
2. **Stray trailing hyphen in skills (e.g. `"PEFT-"`), fixed.** Traced the
   render path first: `resumeJson.skills.join(", ")` in `drawResumePdf()`
   reproduces the artifact exactly when fed `["PEFT-", "HuggingFace
   Transformers"]` — i.e. the join is innocent, the hyphen is already baked
   into the skill string itself before it reaches the renderer. Root cause
   is upstream, almost certainly a PDF-text-extraction artifact from a
   justified-text line-wrap in the source PDF (Call 0 / `EXTRACT_RESUME`)
   that survives into `resumeJson.skills` and then gets carried forward
   through `TAILOR_RESUME`/`COMPRESS_RESUME` since neither of those touches
   skill strings beyond folding new ones in. Fixed defensively with a new
   `sanitizeSkillsArray()` in `background.js` (trims each skill and strips a
   trailing standalone hyphen — never valid in a real skill name, so safe to
   strip unconditionally) applied after parsing in all three: `extractResume`,
   `tailorResume`, and `compressResume` — so the artifact gets cleaned at its
   most likely origin (extraction) and can't survive a future round trip
   even if it re-enters some other way.
3. **Visual hierarchy — investigated, code was already correct.** The report
   described the tailored PDF as flat/no-bold. Rendered the current
   `drawResumePdf()` directly in Node (bypassing the extension entirely,
   using the bundled jsPDF) against a reconstruction of the reported output,
   and the result already has full hierarchy: bold 17pt name, bold
   underlined section headers, bold job-title/company and project-name
   lines, italic location, normal-weight bullets. `doc.setFont("Helvetica",
   "bold"/"normal"/"italic")` was also independently confirmed to select the
   correct embedded font (checked the actual `/F#` resource → `BaseFont`
   mapping in the generated PDF's object stream) — no case-sensitivity or
   fallback bug there. Most likely explanation: the compared PDF was
   generated by a stale unpacked-extension load in Chrome before a reload
   picked up the current `content.js` (`git log` shows the bold styling was
   already added in a prior "bug fix" commit). **No code change made for
   this one** — if it resurfaces after confirming a fresh `chrome://extensions`
   reload, it needs a new repro, not a re-application of this fix.
4. **Fabricated bullets — fixed with a structural check, not just a prompt
   tweak.** This was the critical one: `TAILOR_RESUME` was inventing new
   bullets wholesale (a RAG/pgvector claim and an LLM-eval claim that don't
   exist in the source) and fully replacing at least one existing bullet's
   underlying claim rather than rephrasing it, despite the system prompt
   already saying not to. Fixed two ways:
   - `TAILOR_RESUME_SYSTEM_PROMPT`'s core rule was rewritten to be
     mechanism-based, not just instructional: it now explicitly requires
     the output `bullets` array for each entry to have the **exact same
     length as the input**, one rephrased output bullet per input bullet,
     same order — framed as something the model should count and confirm
     before finalizing. Added two more bad/good example pairs specifically
     about bullet-count changes and claim-swapping (on top of the existing
     new-project and fabricated-detail examples).
   - **Server-side validation**, since a prompt alone doesn't guarantee
     compliance: new `validateBulletCounts(original, tailored)` in
     `background.js` counts `experience`/`projects` entries and each
     entry's `bullets.length` against the original (matched by index) after
     every `TAILOR_RESUME` response. On a mismatch, `tailorResume()` retries
     **once** with an explicit correction message appended to the original
     prompt naming exactly which entries mismatched and by how much
     (factored the Anthropic fetch/parse logic out into
     `callClaudeForTailoredResume()` so the initial call and the retry share
     one code path). If the retry still mismatches, the function returns
     `success:false` with a clear error instead of silently returning the
     fabricated result — verified this logic directly with a Node unit test
     (fabricated-bullet case correctly flagged invalid, clean rewrite
     correctly passes).
   - This validation pass also now feeds the before/after changes summary:
     `buildBulletChanges()` diffs the two bullet arrays index-by-index once
     validation confirms the counts line up, and `tailorResume()` returns it
     as `bulletChanges` alongside `resumeJson`. `content.js`'s
     `renderResumeDownloadSuccess()` renders it as a collapsible `<details>`
     list ("N bullets rephrased", each with before/after text) on the
     success screen — this didn't exist before this session.
   - **Not yet live-tested against a real Anthropic API call** — the
     validation/retry logic itself was unit-tested in isolation (Node, not
     the extension), but the actual end-to-end behavior (does the retry
     message reliably fix a real model's over-eager rewrite, does a
     legitimately-impossible JD ever exhaust both attempts) has not been
     exercised against the live API yet. This is the next thing to confirm
     before calling the fabrication issue fully closed.
